...
<div class="main">
    ...
    <div class="perfil_box">
        <ul>
            ...
            <li>
                <div class="perfil_box_item">
                    <div class="perfil_avatar">
                        <a href="{{route('perfil.show', $item->id)}}" title="Usar esse perfil">
                        @if ($item->arquivo)
                    ...
                    <div class="perfil_info">
                        ...
                        <div class="perfil_actions">
                            <a class="button_action" href="{{route('perfil.edit', $item->id)}}">Editar</a>
                            <form action="{{route('perfil.destroy', $item->id)}}" method="POST">
                                @csrf
                                @method('DELETE')
                            <button class="button_action danger" type="submit">Excluir</button>
                            </form>
                        </div>
                </div>
...
@section('styles')
    <style>
        ...
        .perfil_box li .perfil_box_item {
            display: flex;
            align-items: center;
            justify-content: center;
        }

        .perfil_box li div.perfil_avatar {
            width: 103px;
            height: 103px;
            display: flex;
            align-items: center;
        ...
        .perfil_box li div.perfil_avatar:hover {
            background: #FFF;
            border-radius: 50%;
        }

        .perfil_box li div.perfil_avatar svg {
            width: 85%;
            height: 85%;
        }
        .perfil_box li div.perfil_box_item p {
            font-size: 32px;
            font-style: normal;
            font-weight: 700;
            line-height: 38px;
        ...
        .perfil_info .perfil_actions {
            display: flex;
            flex-direction: row;
        }

        .perfil_actions .button_action {
            display: flex;
            align-items: center;
            justify-content: center;
            height: 25px;
...
@section('javascript')
    ...
    $('.danger').click(function(e){
        ...
        if (confirm('Tem certeza que deseja remover esse perfil?')) {
            // Post the form
