...
?>
  ...
  @if(sizeof($conteudo_series_favoritos))
  ...
  <div class="favorites-contens">
    <ul class="favorites-slider favorites-slider__video list-inline  slick-slider">
      ...
      @foreach ($conteudo_series_favoritos as $i => $conteudo)
      <?php
          $desc='';
      ...
      ?>
        <li class="slide-item">
            <div class="block-images position-relative" onclick="abrirModal({{isset($conteudo->temporada)?$conteudo->temporada:0}},{{$conteudo->id}},'{{$desc}}','{{$conteudo->getImagemBannerFullUrl()}}','{{env('API_URL')}}')">
              <div class="img-box">
                <img data-original="{{ $conteudo->getImagemFullUrl() }}" src="{{ asset('/geral/lazy.png') }}" class="img-fluid" alt="{{ $conteudo->titulo }}">
              </div>
            </div>
            <div class="block-description">
              <div class="botoes-card">
                <!--
              <a href="{{ route('video.series', isset($conteudo) ? $conteudo->id : '') }}">
            ...
            -->
              <button class="btn-fav" aria-label="Favoritar" data-balloon-pos="up" onclick="favorite('serie', {{$conteudo->id}});">
                  <span class="btn-fav-hover">
                      <i class="glyphicon {{$conteudo->liked($perfilAtual) ? 'glyphicon-ok' : 'glyphicon-plus'}} favorite_{{$conteudo->id}}" aria-hidden="true"></i>
                  </span>
              </button>
              <button class="btn-mais" aria-label="Mais Informações" data-balloon-pos="up" onclick="abrirModal({{isset($conteudo->temporada)?$conteudo->temporada:0}},{{$conteudo->id}},'{{$desc}}','{{$conteudo->getImagemBannerFullUrl()}}','{{env('API_URL')}}')">
  ...
  @endif
    ...
    <div class="iq-main-header d-flex justify-content-between">
        <h4 class="main-title">
            <a
                href="{{ route('home_series_categoria', ['category' => $by_category]) }}">{{ $categorias_indexadas[$by_category]->descricao }}</a>
        </h4>
    </div>
      <div class="favorites-contens">
      ...
      ?>
        <li class="slide-item">
            <div class="block-images position-relative" onclick="abrirModal({{isset($conteudo->temporada)?$conteudo->temporada:0}},{{$conteudo->id}},'{{$desc}}','{{$conteudo->getImagemBannerFullUrl()}}','{{env('API_URL')}}')">
              ...
              <div class="botoes-card">
              <a href="{{ route('video.series', isset($conteudo) ? $conteudo->id : '') }}">
                <button class="btn-play" aria-label="Assistir" data-balloon-pos="down" style="display:none;">
                  <span class="btn-play-hover">
                    <i class="fa fa-play mr-1" aria-hidden="true"></i>
                  </span>
                </button>
